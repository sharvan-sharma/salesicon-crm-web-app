...
function SellersTable(props){
    ...
    const Url=()=>{
        switch(props.screen){
            case 0:return '/adminapi/staff/readall'
    ...
    if(state.loading){
        ...
        return (
            <>
             {
                    (props.screen > 0)?
                        <div className='d-flex bg-black shadow my-2 p-2 fsm justify-content-between rounded align-items-center col-12 col-md-8 col-lg-6'>
            ...
            <div className='col-12 p-0 my-2'>
                ...
                {
                    Object.entries(props.staffsObject).map((item)=><SellersGrid key={item[0]} staff={item[1]} mode={(props.screen > 0)?'report':'non-report'} />)
                }
                {
